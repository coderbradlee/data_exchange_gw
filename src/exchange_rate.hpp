...
{
public:
	curl_base(const std::string& url) :m_url(url),m_request_status(0),m_data(nullptr)
	{
		init();
	}
	virtual ~curl_base()
...
CURLSH* curl_base::share_handle = NULL;
//#define DEBUG
//https://www.exchangerate-api.com/USD/GBP?k=d2408a508ec4f2bec554f465
class exchange_rate:public curl_base
{
public:
	exchange_rate(const std::string& url):curl_base(url){}
...
public:
	...
	string second_first_exchange_rate;
	void print()
	{
...
public:
	...
	boost::shared_ptr<MySql> get_conn()
	{
		if(!m_conn)
			m_conn=boost::shared_ptr<MySql>(new MySql(m_mysql_ip.c_str(), m_mysql_username.c_str(),m_mysql_password.c_str(), m_mysql_database.c_str(), m_mysql_port));
		return m_conn;
	}		
};
class mysql_database_eu:public mysql_database_base
{
public:
	mysql_database_eu()
	{
		m_mysql_ip=get_config->m_mysql_eu_ip;
		m_mysql_port=get_config->m_mysql_eu_port;
		m_mysql_username=get_config->m_mysql_eu_username;
		m_mysql_password=get_config->m_mysql_eu_password;
		m_mysql_database=get_config->m_mysql_eu_database;
	}
};
class mysql_database_os:public mysql_database_base
{
public:
	mysql_database_os()
	{
		m_mysql_ip=get_config->m_mysql_ip;
		m_mysql_port=get_config->m_mysql_port;
		m_mysql_username=get_config->m_mysql_username;
		m_mysql_password=get_config->m_mysql_password;
		m_mysql_database=get_config->m_mysql_database;
	}
};
class mysql_database_js:public mysql_database_base
{
public:
	mysql_database_js()
	{
		m_mysql_ip=get_config->m_mysql_js_ip;
		m_mysql_port=get_config->m_mysql_js_port;
		m_mysql_username=get_config->m_mysql_js_username;
		m_mysql_password=get_config->m_mysql_js_password;
		m_mysql_database=get_config->m_mysql_js_database;
	}
};
class mysql_database_as:public mysql_database_base
{
public:
	mysql_database_as()
	{
		m_mysql_ip=get_config->m_mysql_as_ip;
		m_mysql_port=get_config->m_mysql_as_port;
		m_mysql_username=get_config->m_mysql_as_username;
		m_mysql_password=get_config->m_mysql_as_password;
		m_mysql_database=get_config->m_mysql_as_database;
	}
};
class exchange_rate_refactor:public boost::enable_shared_from_this<exchange_rate_refactor>
{
public:
...
private:
	...
	void get_info_from_myql()
	{
		...
		//select code,currency_id from t_currency
			...
			for (const auto& item : t_currency_tuple_vector)
			{
				...
				{
					m_usd_currency_id=*(std::get<1>(item));
				}				
			}
			for (const auto& item : t_currency_tuple_vector)
			{
				general_rate_data temp;
				temp.second_code=*(std::get<0>(item));
				temp.second_currency_id=*(std::get<1>(item));
				if(temp.first_code=="USD")
					continue;
				temp.first_code="USD";
				temp.first_currency_id=m_usd_currency_id;
				string get_exchange_rate_id = "select currency_exchange_rate_id from t_currency_exchange_rate where source_currency_id=\'"+temp.first_currency_id+"\' and target_currency_id=\'"+temp.second_currency_id+"\'";
				//cout << get_exchange_rate_id << endl;
				string get_exchange_rate_id2 = "select currency_exchange_rate_id from t_currency_exchange_rate where source_currency_id=\'"+temp.second_currency_id+"\' and target_currency_id=\'"+temp.first_currency_id+"\'";
				...
				{
					{
						...
						temp.first_second_currency_exchange_rate_id=*(std::get<0>(i));
					...
					}
					{
						...
						for(const auto& i : exchange_rate_ids)
						temp.second_first_currency_exchange_rate_id=*(std::get<0>(i));
						exchange_rate_ids.clear();
					}
				...
				m_general_rate_data_array.push_back(temp);
			}	
	...
	void merge_info_from_mysql_and_curl()
	{
		try
		{
			...
			get_info_from_myql();
			//SKW TRL RUR PLZ
			for(std::vector<general_rate_data>::iterator item=m_general_rate_data_array.begin();item!=m_general_rate_data_array.end();++item)
			{
				if(item->code=="SKW"||item->code=="TRL"||item->code=="RUR"||item->code=="PLZ"||item->code=="MXP"||item->code=="ROL"||item->code=="11")
				{
					m_general_rate_data_array.erase(item);
				}
			}
			...
			{
				string usditem="USD"+item.first_code;
				cout<<usditem<<":"<<__FILE__<<":"<<__LINE__<<endl;
				if (quotes.find(usditem) == quotes.end()) 
				{
  					cout<<item.first_code<<":"<<__FILE__<<":"<<__LINE__<<endl;
  					continue;
				}
				try
				{
	    			...
	    			item.first_second_exchange_rate=boost::lexical_cast<string>(temp_from_usd_exchange_rate);
					item.second_first_exchange_rate=boost::lexical_cast<string>(temp_to_usd_exchange_rate);
				}
				catch(std::exception& e)
				{
    ...
    {  
        ...
        {  
        	...
        	{
        		merge_info_from_mysql_and_curl();
        		start_update_about_usd();
        		general_update();
	...
	void update_exchange_rate_to_mysql(const general_rate_data& item)
	{
		try
		{
		
			if (item.first_second_currency_exchange_rate_id!="")
			{
				insert_exchange_rate(item.first_second_currency_exchange_rate_id,item.first_second_exchange_rate);
			}
			
		
			if (item.second_first_currency_exchange_rate_id!="")
			{
				insert_exchange_rate(item.second_first_currency_exchange_rate_id,item.second_first_exchange_rate);
			}
...
{
	std::vector<boost::shared_ptr<mysql_database_base>> v;
	v.push_back(boost::shared_ptr<mysql_database_base>(new mysql_database_os()));
	v.push_back(boost::shared_ptr<mysql_database_base>(new mysql_database_js()));
	v.push_back(boost::shared_ptr<mysql_database_base>(new mysql_database_as()));
	v.push_back(boost::shared_ptr<mysql_database_base>(new mysql_database_eu()));
	

	std::vector<boost::shared_ptr<exchange_rate_refactor>> v2;	
	for(const auto& item:v)
	{
		v2.push_back(boost::shared_ptr<exchange_rate_refactor>(new exchange_rate_refactor(item)));
	}
	...
	{
		pool.create_thread([&item](){item->start();});
	}
	pool.join_all();  
}
#endif
